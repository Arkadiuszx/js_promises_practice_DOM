...
const firstPromise = new Promise((resolve, reject)=>{
    document.addEventListener("mousedown", (events)=>{
        if (events.button === 0) resolve("First promise was resolved");
    });
    setTimeout(()=>{
...
const secondPromise = new Promise((resolve, reject)=>{
    document.addEventListener("mousedown", (events)=>{
        if (events.button === 0 || events.button === 2) resolve("Second promise was resolved");
    });
});
const thirdPromise = new Promise((resolve, reject)=>{
    ...
    document.addEventListener("mousedown", (events)=>{
        if (events.button === 0) leftClick = true;
        if (events.button === 2) rightClick = true;
...
firstPromise.then(success).catch(fail);
